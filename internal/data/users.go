...
// Duplicate a custom ErrDuplicateEmail error.
var (
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	// If the password hash is ever nil, this will be due to a logic error in our codebase (probably because we forgot to set a password for the user). It's a useful sanity check to include here, but it's not a problem with the data provided by the client.
...
func (u UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	// Create a slice containing the query arguments. Notice how we use [:] operator to get a slice containing the token hash, rather than passing in the array (which is not supported by the pq driver), and that we pass the current time as the value to check against the token expiry.
